...
# model = DQN(CustomAZPolicy, env, n_steps=131072, batch_size=4096, verbose=1) # init training
model1 = DQN.load("C:\\Users\\tiany\\Documents\\best_model_DQN.zip")  # resume training
# model2 = DQN.load(
#     "C:\\Users\\tiany\\Documents\\best_model_DQN_vs_SelfplaySmarterPlayer.zip"
# )  # resume training


# Set the environment for the model
myself = ModelPlayer(model1, name="DQN_selfplay", deteministic=False)

render_mode = "human" if verbose >= 2 else "rgb_array"


def play_once(verbose=verbose, first="DQN_selfplay"):
    if first == "DQN_selfplay":
        env = ConnectFourEnv(
            opponent=SelfTrained7Player(deteministic=True),
            render_mode=render_mode,
            first_player=1,
            main_player_name="DQN_selfplay",
        )
    else:
        env = ConnectFourEnv(
            opponent=SelfTrained7Player(deteministic=True),
