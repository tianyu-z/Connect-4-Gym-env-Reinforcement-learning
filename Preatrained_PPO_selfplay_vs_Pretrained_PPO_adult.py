...
model2 = PPO.load(
    "C:\\Users\\tiany\\Documents\\best_model_PPO_vs_AdultSmarterPlayer.zip"
)  # resume training


# Set the environment for the model
myself = ModelPlayer(model1, name="PPO_selfplay", deteministic=False)
opponent = ModelPlayer(model2, name="PPO_adult", deteministic=False)
render_mode = "human" if verbose >= 2 else "rgb_array"


def play_once(verbose=verbose, first="selfplay"):
    if first == "selfplay":
        env = ConnectFourEnv(
            opponent=opponent,
            render_mode=render_mode,
            first_player=1,
            main_player_name="PPO_selfplay",
        )
    else:
        env = ConnectFourEnv(
            opponent=opponent,
...
if __name__ == "__main__":
    ...
    for i in tqdm(range(1000)):
        isAFNwin = play_once(first="PPO_adult")
        if isAFNwin == 1:
            result["win"] += 1
        elif isAFNwin == -1:
            result["lose"] += 1
