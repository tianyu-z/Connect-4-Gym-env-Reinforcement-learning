...
class ResNet(nn.Module):
    def __init__(self, num_blocks, num_channels):
        ...
        self.res_blocks = nn.ModuleList(
            [ResBlock(num_channels) for i in range(num_blocks)]
        )
...
class SelfTrained6Player(Player):
    def __init__(self, name="SelfTrained6Player", deterministic=False):
    ...
    def play(self, observations):
        ...
        with torch.no_grad():
            pred_policies, pred_values = self.alphaFour.model(
                self.alphaFour.obs_to_tensor(observations)
            )
