...
from ours.our_env import BaseEnv, Player
from ours.our_eval import Agent
...
class AZResNet(nn.Module, Agent):
    def __init__(
        self,
        ...
        self.policy_head_0 = PolicyBlock(
            np.prod(state_dims[1:]), action_dims, res_filters, head_filters
        )
        self.value_head_0 = ValueBlock(
            np.prod(state_dims[1:]), res_filters, head_filters
        )

        self.policy_head_1 = PolicyBlock(
            np.prod(state_dims[1:]), action_dims, res_filters, head_filters
        )
        self.value_head_1 = ValueBlock(
            np.prod(state_dims[1:]), res_filters, head_filters
        )
