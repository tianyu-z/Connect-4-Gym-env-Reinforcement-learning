...
# Load or train your DQN model
DQN_model_path = "C:\\Users\\tiany\\Documents\\best_model_DQN.zip"  # Specify the path to your trained model
model = DQN.load(DQN_model_path)
NN = ModelPlayer(model, name="DQN", deteministic=False)
...
def play_once(verbose=verbose, first="AFN"):
    if first == "AFN":
        ...
        DQN_connect4 = ConnectFourEnv(
            opponent=NN, render_mode=render_mode, first_player=1, main_player_name="AFN"
        )
        if fix_seed:
            DQN_connect4.seed(SEED)
        # Game loop
        obs, _ = DQN_connect4.reset()
        AFN_connect4.reset()
        while True:
            ...
            if done:
                break
            obs, rewards, done, truncated, info = DQN_connect4.step(AFN_action)
            DQN_action = info["opponent_action"]
            if done:
                break
            obs, done = AFN_connect4.step(DQN_action)
            if done:
                break
